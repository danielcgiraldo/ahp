    ...
    */
    ...
    WeightFromMacro.addCriteria(
        [
            [
                1,
                body.weights.char_price_safety,
                1 / body.weights.char_design_price,
            ],
            [
                1 / body.weights.char_price_safety,
                1,
                body.weights.char_safety_design,
            ],
            [
                body.weights.char_design_price,
                1 / body.weights.char_safety_design,
                1,
            ],
        ],
        "char",
    ...
    AHPCalc.setAlternatives(NormalizedCriteriaData[0]);
    const result = AHPCalc.run();
    return new Response(
        JSON.stringify({
            status: "ok",
            result: result,
            normalized: NormalizedCriteriaData[0],
            linear: NormalizedCriteriaData[1],
...
class MacroCriteria {
    ...
    addCriteria(
        subCriteriaMatrix: number[][],
        macroCriteriaKey: string,
        subCriteriaKeys: string[]
    ) {
        const localSubCriteriaWeights = this.getLocalWeights(subCriteriaMatrix);
        for (let i = 0; i < localSubCriteriaWeights.length; i++) {
            this.Criteria[subCriteriaKeys[i]] =
                localSubCriteriaWeights[i] *
                this.MacroCriteria[macroCriteriaKey];
        }
    }
